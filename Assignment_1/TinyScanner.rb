...
#   @c        - A one character lookahead 
    ...
    #begin
        if (!File.exist?(filename))
            puts "File cannot be opened or doesn't exist"
            exit
        else
        @f = File.open(filename,'r:utf-8')
        #raise "File not able to be opened"
    #end
	...
	def nextToken() 
        ...
        if @c == "eof"
            ...
            nextCh()
            return Token.new(Token::LPAREN, lparen)
        elsif (@c == rparen)
            nextCh()
            return Token.new(Token::RPAREN, rparen)
        elsif (@c == "+")
            nextCh()
            return Token.new(Token::ADDOP, "+")
        elsif (@c == "*")
            nextCh()
            return Token.new(Token::ASTER, "*")
        elsif (@c == "/")
            nextCh()
            ...
            return Token.new(Token::NUM, number)

        else
            nextCh()
            return Token.new(Token::UNKNOWN, "unknown")
